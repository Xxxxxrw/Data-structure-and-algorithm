...
void BinTreeThreading_LDR(ThreadBinTree *bt)     //将二叉树p中序线索化
{
    if(bt) //p非空时，当前访问结点是*p
    {
        ...
        if(Previous) //若p的前趋Previous存在
        {
            if(Previous->rflag==Thread) //若*p的前驱右标志为线索
                Previous->right=bt;//令*Previous的右线索指向后继
            if(bt->lflag==Thread) //*p的左标志为线索
                bt->left=Previous;//令*p的左线索指向中序前驱 
        }
