...
void CreateTree(HuffmanTree *ht,int n,int *w)    //创建赫夫曼树
{
    int i,m=2*n-1;//总的节点数
    int bt1,bt2; //二叉树结点序与 
    ...
    for(i=n+1;i<=m;++i) //逐个计算非叶结点，创建Huffman树 
    {
        ...
        ht[bt1].parent=i;        //设置两个节点的父节点
        ht[bt2].parent=i;
        ht[i].left=bt1;          //设置父节点的两个子节点
        ht[i].right=bt2;
        ht[i].weight=ht[bt1].weight+ht[bt2].weight;   //在父节点中保存权值
    }
}
void HuffmanCoding(HuffmanTree *ht,int n,HuffmanCode *hc)//生成每个字符的赫夫曼编码
{
     char *cd;
     int start,i;
     int current,parent;    
