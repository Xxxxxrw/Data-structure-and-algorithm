#include <stdio.h>
#define ARRAYLEN 10
int source[]={54,90,6,69,12,37,92,28,65,83};   //增加到二叉排序树中的数据
typedef struct bst    //定义二叉树结构
{
    int data;
    struct bst *left;
    struct bst * right;
...
BSTree *SearchBST(BSTree *t,int key)
{
    if(!t || t->data==key) //结点为空，或关键字相等 
        ...
        return(SearchBST(t->right,key));  //在右子树中查找
    else              //关键字小于结点数据
        return(SearchBST(t->left,key));   //在左子树中查找
}
...
int main()
{
    ...
    printf("原数据:"); 
    for(i=0;i<ARRAYLEN;i++)    //显示原数据
        printf("%d ",source[i]);
    printf("\n");
    CreateBST(&bst,source,ARRAYLEN);   //创建二叉排序树
    printf("遍历二叉排序树:"); 
    BST_LDR(&bst);   //遍历输出二叉排序树
    //BST_Delete(&bst,37);   //删除二叉排序树的节点
    //printf("\n删除结点后的结点:"); 
    //BST_LDR(&bst);    
    printf("\n请输入关键字:");
    scanf("%d",&key); 
    pos=SearchBST(&bst,key);  //在二叉排序树中查找
    if(pos)   //若返回节点不为空
