...
void CreateHash(int hash[],int m,int data[],int n)   //创建散列表
{
    int i;
    for(i=0;i<n;i++) //循环将原始数据保存到哈希表中 
        InsertHash(hash,m,data[i]);    //调用函数，将数据插入散列表
}
int HashSearch(int hash[],int m,int key)   //在散列表中查找
{
    int i;
    i=key % 13;//计算哈希地址 
    while(hash[i] && hash[i]!=key) //判断是否冲突 
        i=(++i) % m; //线性探测法解决冲突
    if(hash[i]==0) //没查找到开放单元，表示查找失败 
        return -1;//返回失败值 
    else//查找成功 
        return i;//返回对应元素的下标 
}
