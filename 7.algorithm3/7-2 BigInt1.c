...
void BigIntAdd(pBIGINT num1, pBIGINT num2, pBIGINT result)  //判断两个大整数相加的符号
{
   int i;
...
void BigIntAdd1(pBIGINT num1, pBIGINT num2, pBIGINT result)   //大整数加法函数
{
   int i, carry;
   carry = 0;              //清除进位
   ...
   for (i = 0; i < num2->digit; i++)	//num2中的数小，可能位数会少一些
   {
...
{
   int i, borrow;
   result->minus = num1->minus;	//结果符号(因num1绝对值比num2大，结果符号与num1相同)
   borrow = 0;
...
void BigIntMul(pBIGINT num1, pBIGINT num2, pBIGINT result)  //大整数乘法运算函数
{
   char carry, temp;
   int i, j, pos;
