...
void separator(int n,char ch,char newline) //输出多个相同字符 
{
   int i;
   for(i=0;i<n;i++)
       printf("%c",ch);
...
void PrintDate(struct tm gm_date)   //输出日期结构
{
...
void ShowPark(SeqStack *s) //查看车位状态 
{
   ...
   {
	  printf("位置\t车牌号\t进站时间\n");    //输出表头
	  for (i = 0; i <= s->top; i++)    //循环从栈中读取数据
	  {
		 printf("%d\t", s->car[i].position);   //停靠车位
		 printf("%s\t", s->car[i].num);   //车牌号
...
void InPark(SeqStack * s, CarChainQueue * q) //车辆进入停车场 
{
   ...
   struct tm *gm_date;
   time_t seconds;
   time(&seconds);    //获取系统当前时间（long型）
   gm_date = gmtime(&seconds);;  //转换为tm格式
...
void OutPark(SeqStack *s, CarChainQueue *q) //车辆离开，当时过道上的情况放在过道上直接进入车站
{
   struct tm *gm_date;
   time_t seconds;
   ...
   else
   {
	  ...
	  if (pos > 0 && pos <= s->top + 1) //输入车位号正确 
	  {
		 ...
		 car.length=mktime(&car.outtime)-mktime(&car.intime); //计算停车场中停放时间 
		 car.expense=(car.length/3600+1)*PRICE; //计算费用 
