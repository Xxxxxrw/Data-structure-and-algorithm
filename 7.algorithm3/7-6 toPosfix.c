...
{
   int length=strlen(infix);    //表达式的长度
   ...
   for(i=0;i<length;i++)
   {
	  ...
	  case '*':
	  case '/':
		 while(PRI(stack[top])>=PRI(op)) //判断栈顶运算符与当前运算符的优先级
		 {
...
double eval(char *postfix)		//计算表达式的值 
{
   ...
   length=strlen(postfix);       //获取后序字符串长度
   if(!(stack=(double *)malloc(sizeof(double)*length)))   //分配内存保存栈中的数据
   {
       printf("内存分配失败!\n");
       exit(0); 
   }
   ...
   for(i=0;i<length;i++)
   {
	  ...
	  case '/':
		 ...
		 stack[top-1]=calc(stack[top-1], token, stack[top]);//取出栈中前两个元素进行运算，结果保存到栈中 
		 top--; //修改栈顶指针 
		 dec=0;//先设为整数	
         flag=0;//下一步操作不将数入栈	 
		 break;
