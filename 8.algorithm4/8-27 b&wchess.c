		 ...
		 for (rowdelta = -1; rowdelta <= 1; rowdelta++)	//循环检查上下行 
		 {
			for (coldelta = -1; coldelta <= 1; coldelta++)	//循环检查左右列 
			{
			   ...
			   if (chessboard[row + rowdelta][col + coldelta] == opponent)	//若(row,col)四周有对手下的子 
			   {
				  ...
				  while(1)	//以对手下子为起始点，向四周查找自己方的棋子，以攻击对方棋子 
				  {
...
int GetMaxScore(char chessboard[][8], char player)	//获取指定游戏方在棋盘中的旗子数量
{
   int Score, row, col;
   char opponent = (player == 1) ? -1 : 1;	//对方棋子 
   char myplayer=-1*opponent;
   for (row = 0; row < 8; row++)	//循环计算当前方的棋子数 
   {
	  for (col = 0; col < 8; col++)
	  {
		 Score -= chessboard[row][col] == opponent;	//若棋盘对应位置是对手下的棋子，从总分中减1分 
   ...
   for (row = 0; row < 8; row++)	//循环检查棋盘每个单元格 
   {
	  for (col = 0; col < 8; col++)
	  {
		 ...
		 for (i = 0; i < 8; i++)	//将棋盘原来的棋子复制到临时数组中 
			for (j = 0; j < 8; j++)
			   chessboard1[i][j] = chessboard[i][j];
		 PlayStep(chessboard1, row, col, player);	//试着在临时棋盘中的一个位置下子 
