...
int main()
{
   ...
   for (i = 1; i <= n; i++)	//将盘子放入第1个棒
	  array[i] = height - i; //数字大的表示大盘子 
   for (i = 0; i <= 2 * height; i += height)	//将每个棒的底部设置为一个大的数据 
	  array[i] = 1000;
   if (n % 2 == 0) //若圆盘序号是偶数 
   {
	  target = 1; //目标为B棒 
	  stick[2] = 0;
	  stick[1] = 1;
	  array[height + 1] = array[n]; //移动第1个圆盘到第2个棒 
   }
   else //若圆盘序号为奇数 
   {
	  target = 2; //目标为C棒 
	  stick[1] = 0;
	  stick[2] = 1;
	  array[2 * height + 1] = array[n]; //移动第1个圆盘到第3个棒 
   }
   ...
   while(stick[0] + stick[1])	//第1棒和第2棒不为空,则循环移动
   {
	  if (target == 0) //若目标棒是A 
	  {
          if (array[height + stick[1]] < array[2 * height + stick[2]])//比较B、C棒的较小值作为下一个移动棒 
              source = 1;
          else
              source = 2;
      }
	  if (target == 1)//若目标棒是B
	  {
          if(array[stick[0]] < array[2 * height + stick[2]]) //比较A、C棒的较小值 
              source = 0;
          else
              source = 2;
	  }
	  if (target == 2)//若目标棒是C
	  {
          if(array[stick[0]] < array[height + stick[1]]) //比较A、B棒的较小值 
              source= 0;
          else
              source= 1;
      }
      // 将source棒顶部圆盘移到另一较大或为偶数的棒 
